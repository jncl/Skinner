...
function aObj:oQueue()
	...
	self:skinEditBox{obj=OQ_MinIlevel4, regs={9}}
	...
	-- TabPage5 (Setup)
	self:skinScrollBar{obj=OQTabPage5ListScrollBar}
	self:applySkin{obj=OQTabPage5List} -- applySkin so list items appear in the FG
	self:skinEditBox{obj=OQ_BnetAddress9, regs={9}}
	self:skinButton{obj=OQTabPage5Button9}
	self:skinButton{obj=OQTabPage5Button10}
	self:skinButton{obj=OQTabPage5Button11}
	self:skinButton{obj=OQTabPage5Button12}
	self:skinButton{obj=OQTabPage5Button13}
	self:skinButton{obj=OQTabPage5Button14}
	local function skinAlts()
		for i = 1, 99 do
			if _G["OQTabPage5ListAlt"..i] then
				_G["OQTabPage5ListAlt"..i].texture:SetTexture(nil)
				self:skinButton{obj=_G["OQTabPage5ListAlt"..i].remove_but, as=true} -- applySkin so text appears in the FG
			end
		end
	end
