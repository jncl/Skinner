	...
	local function skinSanityBags()
		...
		for _, v in pairs(SanityBags.bags) do
			if not v.frame.skinned then
				...
				Skinner:Hook(sbt, "SetTexture", function() end, true)
				local sbbgt = _G[sb:GetName().."BagFullFrameBGTexture"]
				Skinner:Hook(sbbgt, "SetTexture", function() end, true)
				Skinner:skinScrollBar(StatusBar)
				Skinner:Hook(v.frame, "SetBackdropColor", function() end, true)
				Skinner:Hook(v.frame, "SetBackdropBorderColor", function() end, true)
