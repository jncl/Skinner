...
function aObj.removeColourCodes(_, text)
	...
	if text then
		...
		for _, aCode in _G.pairs{"\124\99", "\124\67"} do -- if string contains colour prefix [7C 63/43] |c & |C
			sPos, ePos = newText:find(aCode)
			while sPos do
				if newText:match("n%D+_COLOR:") then -- if string contains a color name, e.g. nPURE_BLUE_COLOR:
					newText = newText:gsub(aCode .. "n%D+_COLOR:", "") -- remove colour name
				else
					newText = newText:gsub(aCode .. "%x%x%x%x%x%x%x%x", "") -- remove hex digits
				end
				newText = newText:gsub("\124\114", "") -- remove colour suffix [7C 72] |r
				sPos, ePos = newText:find(aCode, ePos + 1)
			end
		end
